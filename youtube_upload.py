...
class YoutubeUploader:
    def __init__(self, file, title, description="Training exercise", keywords=None, category=22, privacyStatus='unlisted'):
        self.file = file
        self.title = title
    ...
    def upload(self):
        ...
        response = None
        try:
            response = self.do_video_upload(youtube)
    ...
    # do the video upload
    def do_video_upload(self, youtube):
        body = dict(snippet=dict(title=self.title,
                    description=self.description, tags=self.tags,
                    categoryId=self.category),
                    status=dict(privacyStatus=self.privacyStatus))

    # call the API's videos.insert method to create and upload the video.
        insert_request = \
            youtube.videos().insert(part=','.join(body.keys()), body=body,
                                    media_body=MediaFileUpload(self.file,
